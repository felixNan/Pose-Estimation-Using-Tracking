...
%posSizes4Tracker = [0.1, 0.15, 0.2, 0.3];
posSizes4Tracker = 0.05;

formatSpec4date = 'mm_dd_yy_HH_MM_SS';

for p = 1:length(posSizes4Tracker)
    posSizeString = sprintf('_%f_', posSizes4Tracker(p));
    for q = 1: size(dateVectors, 1)
        dateVector = dateVectors(q,:);
        relevantJoints = 8;
        doRunAlg = true;
        ...
        dateOfRunString = ['_run_date_' datestr(now, formatSpec4date)];
        outputVideoFileNameAlg = [ 'outputVideoAlg' dateStringMat dateOfRunString posSizeString '.avi'];
        metricsMatFile = ['metricResults' dateStringMat dateOfRunString posSizeString '.mat'];
